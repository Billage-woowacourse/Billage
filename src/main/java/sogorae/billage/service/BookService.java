...
public class BookService {
    ...
    public void requestLent(Long bookId, String email, String requestMessage) {
        ...
        mailSenderService.send(book.getMember().getEmail(),
            client.getNickname() + "님이 " + book.getTitle().replaceAll("<[^>]*>", " ") + "책을 빌림 요청했습니다. " + "요청 메세지: "
                + lent.getRequestMessage());
    }
    ...
    private void allowLent(Long bookId, String email) {
        ...
        lent.updateLent();
        mailSenderService.send(lent.getClient().getEmail(),
            owner.getNickname() + "님이 " + book.getTitle().replaceAll("<[^>]*>", " ") + "책을 빌림 요청을 수락했습니다.");
    }
    ...
    private void denyLent(Long bookId, String email) {
        ...
        mailSenderService.send(lent.getClientEmail(),
            book.getMember().getNickname() + "님이 " + book.getTitle().replaceAll("<[^>]*>", " ") + "책을 빌림 요청을 거절했습니다.");
    }
